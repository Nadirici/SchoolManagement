...
// Notification par email
    ...
    @PostMapping("/{adminId}/courses/update/{courseid}")
    public String updateCourse(@PathVariable("courseid") String courseId,
                               @PathVariable("adminId") String adminId,
                               @RequestParam("name") String courseName,
                               @RequestParam("description") String courseDescription,
                               @RequestParam("teacherId") String teacherId,
                               @RequestParam("dayOfWeek") String newDay,
        ...
        if (courseId != null && !courseId.isEmpty()) {
            ...
            try {
                ...
                if (course.isPresent()) {
                    ...
                    // Mettre à jour le cours avec seulement le nom et la description, si d'autres champs ne sont pas modifiés
                    courseService.addCourse(course.get());

                    // Si l'enseignant, le jour, ou les horaires sont modifiés, faire les vérifications
                    if (teacherId != null && !teacherId.isEmpty() && newDay != null && !newDay.isEmpty() &&
                            newStartTime != null && !newStartTime.isEmpty() && newEndTime != null && !newEndTime.isEmpty()) {
                        ...
                        Optional<TeacherDTO> teacher = teacherService.getTeacherById(UUID.fromString(teacherId));
                        if (teacher.isPresent()) {
                            Teacher teacher1 = new Teacher();
                            teacher1.setDepartment(teacher.get().getDepartment());
                            teacher1.setId(teacher.get().getId());
                            teacher1.setSalt(teacher.get().getSalt());
                            teacher1.setLastname(teacher.get().getLastname());
                            teacher1.setFirstname(teacher.get().getFirstname());
                            teacher1.setEmail(teacher.get().getEmail());
                            teacher1.setPassword(teacher.get().getPassword());
                            teacher1.setVerified(true);

                            Course tempCourse = new Course();
                            tempCourse.setTeacher(teacher1);
                            tempCourse.setId(id);
                            ...
                            if (!teacherService.isAvailable(teacher1, tempCourse)) {
                                return "redirect:/admin/"+adminId+"/courses/" + id+"?flashMessage=teacherNotAvailable";
                            }

                            // Vérifier la disponibilité des étudiants
                            for (Enrollment enrollment : course.get().getEnrollments()) {
                                ...
                                if (!studentService.isStudentAvailable(student, tempCourse)) {
                                    return "redirect:/admin/"+adminId+"/courses/" + id+"?flashMessage=studentNotAvailable";
                                }
                            }

                            // Mettre à jour les autres informations du cours si les vérifications sont passées
                            course.get().setTeacher(teacher1);
                            course.get().setDayOfWeek(dayOfWeekEnum); // Mettre à jour le jour
                            course.get().setStartTime(LocalTime.parse(newStartTime)); // Mettre à jour l'heure de début
                            course.get().setEndTime(LocalTime.parse(newEndTime)); // Mettre à jour l'heure de fin

                            courseService.addCourse(course.get());
                            return "redirect:/admin/"+adminId+"/courses/" + course.get().getId();
                        } else {
                            return "redirect:/admin/"+adminId+"/courses/" + id+"?flashMessage=teacherNotFound";
                        }
                    }

                    return "redirect:/admin/"+adminId+"/courses/" + course.get().getId();
                } else {
                    redirectAttributes.addFlashAttribute("flashMessage", "courseNotFound");
                    return "redirect:/admin/"+adminId+"/courses/"+ id;
                }
            } catch (IllegalArgumentException e) {
                redirectAttributes.addFlashAttribute("flashMessage", "invalidDayOfWeek");
                return "redirect:/admin/"+adminId+"/courses/" + courseId;
            } catch (EntityNotFoundException e) {
                redirectAttributes.addFlashAttribute("flashMessage", "courseNotFound");
                return "redirect:/admin/"+adminId+"/courses/"+ id;
            } catch (Exception e) {
                redirectAttributes.addFlashAttribute("flashMessage", "errorUpdatingCourse");
                return "redirect:/admin/"+adminId+"/courses/"+ id;
            }
        } else {
            redirectAttributes.addFlashAttribute("flashMessage", "invalidCourseId");
            return "redirect:/admin/"+adminId+"/courses/"+ courseId;
        }
    }
